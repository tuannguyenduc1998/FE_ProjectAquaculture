...
class CalendarHeaderComponent {
  ...
  createYearMonthDaySelectors() {
    ...
    if (this.dateHelper.relyOnDatePipe) {
      yearFormat = this /** @type {?} */.dateHelper
        .transCompatFormat(yearFormat);
    }
    year = {
    ...
    if (this.dateHelper.relyOnDatePipe) {
      dayFormat = this /** @type {?} */.dateHelper
        .transCompatFormat(dayFormat);
    }
...
/** @nocollapse */
CalendarHeaderComponent.ctorParameters = () => [{ type: DateHelperService }];
...
class CalendarInputComponent {
  ...
  checkValidInputDate(event) {
    ...
    /** @type {?} */
    const date = new CandyDate(
      window.myParseDate ? window.myParseDate(input, this.format) : input
    );
    this.invalidInputClass = '';
    ...
    if (
      date.isValid() &&
      (input === this.toReadableInput(date) ||
        input === this.toReadableInput1(date) ||
        input === this.toReadableInput2(date))
    ) {
      return date;
    }
    this.invalidInputClass = `${this.prefixCls}-input-invalid`;
  ...
  toReadableInput1(value) {
    const format = 'd/M/yyyy';
    return value ? this.dateHelper.format(value.nativeDate, format) : '';
  }
  toReadableInput2(value) {
...
/** @nocollapse */
CalendarInputComponent.ctorParameters = () => [{ type: DateHelperService }];
...
class TodayButtonComponent {
  ...
  ngOnChanges(changes) {
    if (changes.disabledDate) {
      this.isDisabled =
        this.disabledDate && this.disabledDate(this.now.nativeDate);
    }
    if (changes.locale) {
      // NOTE: Compat for DatePipe formatting rules
      /** @type {?} */
      ...
      if (this.dateHelper.relyOnDatePipe) {
        dateFormat = this /** @type {?} */.dateHelper
          .transCompatFormat(dateFormat);
      }
...
/** @nocollapse */
TodayButtonComponent.ctorParameters = () => [{ type: DateHelperService }];
...
function getTimeConfig(value, disabledTime) {
  ...
  let disabledTimeConfig = disabledTime
    ...
    : /** @type {?} */ ({});
  disabledTimeConfig = Object.assign(
    {},
    defaultDisabledTime,
    disabledTimeConfig
  );
...
class DateRangePopupComponent {
  ...
  onSelectTime(value, partType) {
    if (this.isRange) {
      ...
      newValue[index] = /** @type {?} */ (this.overrideHms(
        value,
        newValue[index]
      ));
      ...
      this.setValue(
        /** @type {?} */ (this.overrideHms(
          value,
          /** @type {?} */ (this.value) || new CandyDate()
        ))
      ); // If not select a date currently, use today
    }
  }
  /**
   * @param {?} value
  ...
  changeValueFromInput(value, partType) {
    ...
    if (this.isRange) {
      ...
      let newRangeValue =
        partType === 'left'
          ? [date, this.selectedValue[1]]
          : [this.selectedValue[0], date];
      /** @type {?} */
      ...
      this.setValueFromInput(
        this.cloneRangeDate(newRangeValue),
        isEnter && isValidRange
      );
    } else {
  ...
  enablePrevNext(direction, partType) {
    if (this.isRange) {
      ...
      if (
        (partType === 'left' && direction === 'next') ||
        (partType === 'right' && direction === 'prev')
      ) {
        return showMiddle;
      }
      return true;
  ...
  getValueBySelector(partType) {
    if (this.isRange) {
      /** @type {?} */
      const valueShow = this.showTimePicker
        ? this.value
  ...
  hasSelectedValue() {
    return (
      this.selectedValue && !!this.selectedValue[1] && !!this.selectedValue[0]
    );
  }
  /**
   * @return {?}
  ...
  okDisabled() {
    ...
    if (this.isRange) {
      return (
        !this.isAllowedSelectedValue() ||
        !this.hasSelectedValue() ||
        !!this.hoverValue.length
      );
    } else {
      return this.value
        ? !isAllowedDate(
  ...
  normalizeRangeValue(value) {
    ...
    /** @type {?} */
    const newEnd =
      end && end.isSameMonth(newStart)
        ? end.addMonths(1)
        : end || newStart.addMonths(1);
  ...
  setRangeValue(partType, value) {
    /** @type {?} */
    const ref = (this.selectedValue = this.cloneRangeDate(
    ...
    for (let rowIndex = 0; rowIndex < MAX_ROW$1; rowIndex++) {
      ...
      for (let colIndex = 0; colIndex < MAX_COL$1; colIndex++) {
        ...
        const disabled = this.disabledDate
          ? this.disabledDate(this.value.setYear(year).nativeDate)
          : false;
        /** @type {?} */
...
class NzPickerComponent {
  ...
  focus() {
    if (this.isRange) {
      /** @type {?} */
      const firstInput = /** @type {?} */ (this.pickerInput /** @type {?} */.nativeElement
  ...
  onPositionChange(position) {
    this.dropdownAnimation =
      position.connectionPair.originY === 'top' ? 'bottom' : 'top';
  ...
  getReadableValue(partType) {
    ...
    if (this.isRange) {
      value = /** @type {?} */ (this.value)[
        this.getPartTypeIndex(/** @type {?} */ (partType))
      ];
    } else {
      value = /** @type {?} */ (this.value);
    }
  ...
  getPlaceholder(partType) {
    return this.isRange
      ? this.placeholder[this.getPartTypeIndex(/** @type {?} */ (partType))]
      : /** @type {?} */ (this.placeholder);
  }
  /**
   * @param {?} value
...
NzPickerComponent.propDecorators = {
  ...
  cdkConnectedOverlay: [
    { type: ViewChild, args: [CdkConnectedOverlay, { static: false }] }
  ],
  ...
  ngOnChanges(changes) {
    if (changes.nzPopupStyle) {
      ...
      this.nzPopupStyle = this.nzPopupStyle
        ? Object.assign({}, this.nzPopupStyle, POPUP_STYLE_PATCH)
        : POPUP_STYLE_PATCH;
    }
    // Mark as customized placeholder by user once nzPlaceHolder assigned at the first time
    if (
      changes.nzPlaceHolder &&
      changes.nzPlaceHolder.firstChange &&
      typeof this.nzPlaceHolder !== 'undefined'
    ) {
  ...
  setDefaultPlaceHolder() {
    if (!this.isCustomPlaceHolder && this.nzLocale) {
      this.nzPlaceHolder = this.isRange
        ? this.nzLocale.lang.rangePlaceholder
        : this.nzLocale.lang.placeholder;
    }
  }
...
__decorate(
  [InputBoolean(), __metadata('design:type', Boolean)],
  AbstractPickerComponent.prototype,
  'nzAllowClear',
  void 0
);
__decorate(
...
__decorate(
  [InputBoolean(), __metadata('design:type', Boolean)],
  DateRangePickerComponent.prototype,
  'nzShowToday',
  void 0
);
if (false) {
...
class HeaderPickerComponent extends AbstractPickerComponent {
  ...
  ngOnInit() {
    ...
    this.supportPanels = allHeaderPanels.slice(
      0,
      allHeaderPanels.indexOf(this.endPanelMode) + 1
    );
  }
  /**
   * @param {?} changes
...
NzDatePickerComponent.ctorParameters = () => [
  ...
  {
    type: NzNoAnimationDirective,
    decorators: [{ type: Host }, { type: Optional }]
  }
];
if (false) {
  /** @type {?} */
...
export {
  ...
  NzWeekPickerComponent,
  NzYearPickerComponent,
  LibPackerModule as ɵa,
  CalendarHeaderComponent as ɵb,
  CalendarInputComponent as ɵc,
  CalendarFooterComponent as ɵd,
  OkButtonComponent as ɵe,
  TimePickerButtonComponent as ɵf,
  TodayButtonComponent as ɵg,
  YearPanelComponent as ɵh,
  MonthPanelComponent as ɵi,
  DecadePanelComponent as ɵj,
  InnerPopupComponent as ɵk,
  DateRangePopupComponent as ɵl,
  DateRangePickerComponent as ɵm,
  AbstractPickerComponent as ɵn,
  NzPickerComponent as ɵo,
  HeaderPickerComponent as ɵp
};
